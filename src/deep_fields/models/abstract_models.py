...
from deep_fields.models.basic_utils import (all_metrics_to_floats, dict_mean, set_cuda)
...
class DeepBayesianModel(torch.nn.Module):
    ...
    def __init__(self, model_name, model_dir=None, data_loader: ADataLoader = None, metalearning=None, evaluate_mode=False, **kwargs):
        ...
        if model_dir is not None:
            ...
            kwargs = json.load(open(self.parameter_path, "r", encoding='utf-8'))
            self.set_parameters(**kwargs)
            self.data_loader = data_loader
            self.define_deep_models()
            self.load_model()
            try:
                self.dataloader_parameters = json.load(open(self.dataloaders_parameters_path, "r", encoding='utf-8'))
                self.inference_parameters = json.load(open(self.inference_parameters_path, "r", encoding='utf-8'))
            except:
                pass
        else:
            ...
            if data_loader is not None:
                ...
                self.set_parameters(**kwargs)
                json.dump(kwargs, open(self.parameter_path, "w", encoding='utf-8'))
                self.model_parameters = kwargs
                self.data_loader = data_loader
                self.define_deep_models()
                try:
                    json.dump(data_loader.dataset_kwargs, open(self.dataloaders_parameters_path, "w", encoding='utf-8'))
                except Exception as e:
                    print("Data Loader has No Parameters", e)
    ...
    def _early_stop(self) -> bool:
        lr = self.optimizer[0].param_groups[0]['lr'] if isinstance(self.optimizer, list) else self.optimizer.param_groups[0]['lr']
    ...
    def inference(
        ...
        **inference_parameters,
        ...
        if self.INFERENCE:
            ...
            with open(self.inference_history_path, "a+", encoding='utf-8') as fh:
                with open(self.inference_path, "a+", encoding='utf-8') as fr:
                    try:
                        for epoch in tqdm(range(1, self.number_of_epochs + 1), desc="Train Epoch", unit='epoch'):
                            self.inference_step(self.optimizer, data_loader, epoch, None, **inference_parameters)
                            model_evaluation = self.validation_step(self.optimizer, data_loader, epoch, None, **inference_parameters)
                            ...
                            if current_eval < self.best_eval:
                                ...
                                json.dump(self.inference_results, fr)
                                fr.write("\n")
                                fr.flush()
                                self.save_model()
                            else:
                        ...
                        if hparam_dict is not None:
                            hparam_dict["epoch"] = self.inference_results["epoch"]
                            self.writer.add_hparams(hparam_dict, {self.best_eval_key: self.inference_results[self.best_eval_key]})
    ...
    def generate_training_message(self):
        ...
        print(f"# NUMBER OF PARAMETERS TO TRAIN {self.count_parameters():,}")
